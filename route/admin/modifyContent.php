...
$input = $_REQUEST['admin'];
$contentID = $_REQUEST['contentID'];
$db = new DB;

if($input == 'delete'){
	$resultDelete = $db->deleteContent($contentID);
	if($resultDelete){
		$db->dbCommit();
		echo "<script>alert('해당 게시물이 정상적으로 삭제 되었습니다.') \n";
		echo "window.location = '/'";
		echo "</script>";
	}
}
...
$resultContentsInfo = $db->getContentDetail($contentID);
?>
<!DOCTYPE html>
<html lang="ko">
<head>
	...
	<div class="wrap">
		<div>
			<header class="primary">
				<hgroup><h1>Modify</h1></hgroup>
			</header>
		</div>
		<form method="post" name="multiple_upload_form" id="multiple_upload_form" enctype="multipart/form-data" action="/admin/modifyContentProc">
			<div class="upload-ncotents">
				<input type="hidden" id="contentID" name="contentID" value="<?=$contentID?>">
				<input type="hidden" id="thumbURL" name="thumbURL" value="<?=$resultContentsInfo['THUMBNAIL_URL']?>">
...
<script>
	...
	function cancel(){
		var confirmYn = confirm('현재 수정중인 문서를 취소하시겠습니까?');
		if(confirmYn){
			history.back();
			return;
		} else {
    ...
    $(document).ready(function() {
		...
		$('#sectionID').val('<?=$resultContentsInfo['SECTION_ID']?>');
		$('#contentType').val('<?=$resultContentsInfo['CONTENT_TYPE']?>');
		$('#contentType').attr('disabled','disabled');
		$('#title').val('<?=$resultContentsInfo['TITLE']?>');
		$('#privateYn').val('<?=$resultContentsInfo['PRIVATE_YN']?>');
		$('#tags').val('<?=$resultContentsInfo['TAGS']?>');
		$('#description').val('<?=$resultContentsInfo['DESCRIPTION']?>');
